...
case "${1:-}" in
  ...
  attribution)
    git diff --check
    if git log --format='%s%n%b' 27e6e2e..HEAD | rg -i 'co-authored-by|generated by|claude|codex|openai|anthropic|gpt-[0-9]'; then
      echo 'implementation metadata contains attribution' >&2; exit 1
    fi
    ;;
