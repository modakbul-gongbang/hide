...
const commands = ['check-hide-theme-literals.mjs', 'check-hide-components.mjs', 'check-design-controls.mjs'];
function run(root) {
  ...
  if (args[0] === '--staged') {
    ...
    for (const entry of entries) {
      ...
      const input = (file.startsWith('macos/Sources/HerdrMacOS/') && file.endsWith('.swift'))
        || [...commands, 'swift-source-tokens.mjs', 'design-control-policy.json'].some(name => file === 'scripts/' + name);
      if (!input) continue;
