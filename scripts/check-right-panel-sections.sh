#!/usr/bin/env bash
# The right panel presents exactly the file explorer and the changes view, and
...
if grep -rniE 'workbench' macos/Sources/HerdrMacOS >/dev/null; then
    printf 'the Workbench name is still present in the shell sources:\n' >&2
    grep -rniE 'workbench' macos/Sources/HerdrMacOS >&2
    exit 1
fi

bash scripts/swift-test.sh ChangesPresentationTests

printf 'right panel presents exactly Explorer and Changes; no Workbench string remains\n'
