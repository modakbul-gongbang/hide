...
impl IndexService {
    ...
    pub fn query(&self, root: &Path, query: &str) -> IndexAnswer {
        let entry = {
            let mut entries = self
                .entries
                .lock()
...
fn build(root: &Path) -> IndexData {
    ...
    builder
        ...
        .git_global(true)
        .git_exclude(true)
        // A checkout's ignore files apply whether or not its `.git` directory
        // is present (a worktree can be handed to hided before git runs).
        .require_git(false)
        .parents(false)
        // The repository's own directory is never a search result, the same
        // name the Explorer hides (D-01).
