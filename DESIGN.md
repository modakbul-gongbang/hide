...
A lane header reads left to right as branch, checkout badges (`detached`, `missing`, `primary`), then the track; the whole row is the open control, disabled for a missing worktree, and only a filled `PR` chip keeps a click of its own.
