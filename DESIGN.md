...
The Needs You strip sits first on the page: every Needs You and Done card across the lanes as `Home.attentionCardWidth` compact cards, in the core's order, wrapped into rows with the count in its title, so the glance answer is the first thing drawn and none of it is hidden behind a sideways scroll.
...
The header carries the project name with the overlay's close control on the title row, then the four group counts in the pet badge colours with a zero kept in place and muted so the row never shifts, the shared `HideSearchField` that narrows cards and lanes (a lane whose own name matches keeps every card; otherwise a matching card keeps its ancestors), and `Start new terminal` as the prominent action; the three wrap under each other when the page is narrow, and a count never breaks inside its word.
