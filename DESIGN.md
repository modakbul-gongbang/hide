...
The scale-1 separation cannot see that a fitted label is drawn larger than the space it cleared, so a final pass resolves the remaining crossings in screen space at the fitted scale: of any two labels that still cross, the lower-priority one is dropped and returns on hover or zoom, priority running hub, Needs You, Done, then Working or Seen, and a hub never dropped. `ProjectHomeGraphLayout.visibleLabels` decides it from `ProjectHomePresentation.labelPriority`.
