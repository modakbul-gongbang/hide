...
`open_external` is the fallback for a document past the editable cap (D-12): the path passes the same boundary check and must be a regular file, and the daemon runs the host's opener (macOS `open`, Windows `ShellExecuteW` default-file association, Linux `xdg-open`, or a validated absolute `HIDE_OPEN_COMMAND`) and answers `open_external_result`.
The Windows association preserves the default-app action without passing a checkout filename through `cmd`.
At most four opener requests may run at once and at most twelve launches are accepted per minute; a spawned CLI helper is reaped or killed within ten seconds or when the daemon stops, while Windows `ShellExecuteW` returns after handing the document to its registered application; crossing a cap answers `over_budget`.
