...
On Unix, its Git commands run from the daemon's opened checkout directory; it compares the opened checkout and registered-folder identities with their current paths before and after reading, so a replaced registration becomes unavailable without redirecting Git to another repository.
