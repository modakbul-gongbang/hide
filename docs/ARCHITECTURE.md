...
The remote oversized-file fallback asks for 4 MiB ranges and writes each into a browser-selected file when a streaming file writer is available; it does not accumulate an unbounded download in the page.
Without that browser API, it retains the existing Blob download for one protocol read of at most 256 MiB and reports a larger read's refusal beside the button.
`open_external` is the fallback for a document past the editable cap (D-12): the path passes the same boundary check and must be a regular file, and the daemon runs the host's opener (macOS `open`, Windows `ShellExecuteW` default-file association, Linux `xdg-open`, or a validated absolute Unix `HIDE_OPEN_COMMAND` CLI helper) and answers `open_external_result`.
...
On Unix, a private `hided` supervisor starts each CLI opener in its own process group and watches a descriptor held only by the daemon, so normal stop, timeout, or daemon death including `SIGKILL` ends the still-running CLI helper and its descendants.
An explicit `HIDE_OPEN_COMMAND` is a CLI helper and its remaining process group is ended when that helper returns; the normal `open` or `xdg-open` exit is treated as the default-application handoff, so a registered application is not closed after that handoff.
