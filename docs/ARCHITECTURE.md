...
Trash first moves the selected item into a private staging directory through the opened parent handle, then passes that staged path to the platform Trash API; a failed handoff attempts to restore the item through the same handles.
`file_index` walks opened directory handles and checks returned paths again before sending them.
Explorer watch polling also retains opened folder handles, so a later pathname replacement does not redirect its observation.
