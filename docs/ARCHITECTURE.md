...
`open_external` is the fallback for a document past the editable cap (D-12): the path passes the same boundary check and must be a regular file, and the daemon runs the host's opener (macOS `open`, Windows `start`, Linux `xdg-open`, or `HIDE_OPEN_COMMAND`) detached, answering `open_external_result`; the frame is a page's request rather than the operator's own click, so the shell's rule holds here too and a program, an application bundle or an installer is refused with `not_openable` instead of launched; a viewer whose daemon is on another machine downloads the bytes through `file_bytes` instead.
