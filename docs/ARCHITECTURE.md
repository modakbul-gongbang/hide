...
The boundary pins each registered root's physical path and Unix file identity across repeated snapshots and refuses an Explorer request when the root's current spelling no longer resolves to that pin.
`file_list` opens relative to a verified root handle and enumerates the resulting directory handle on Unix, then filters each child against the same pin, so replacing the pathname during enumeration cannot expose an outside directory's rows.
`file_index` checks returned paths again before sending them, and watch notifications are sent only while their paths still resolve under a current registered root.
