...
The path is tested as written before the filesystem is read, so any path outside home gets `outside_home` whether or not it exists; a path under home is then resolved by real path so a symlink that leaves home is caught, and when the path stops resolving past such a symlink the reason is `outside_home` too, not `not_found` or `not_a_directory`, so a client holding the token cannot probe the rest of the disk for existence through a symlink under home either.
