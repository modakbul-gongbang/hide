...
A session a Project's history listed and a later read no longer finds stays listed as unavailable, with its last location, for as long as the daemon runs, so a moved or deleted file reads differently from one that never existed (B5); the core keeps what each Project's history listed for that, which is bounded by what the catalog can list.
