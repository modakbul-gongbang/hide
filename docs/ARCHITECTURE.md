...
The normal `open` or `xdg-open` launch is an OS default-application handoff as soon as the utility starts, since `xdg-open` can stay attached to the application for its lifetime; the daemon does not signal that process or its descendants after acceptance.
