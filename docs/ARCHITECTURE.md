...
The Explorer, file tabs and History run on the device's own helper; reorder, reopen closed, find in pane, worktree creation and deletion, and pin stay this machine's and are refused or disabled with the reason; a remote checkout's purpose is written through the core when the host's Herdr is 0.9.1 or newer.
