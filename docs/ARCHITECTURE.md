...
The core words each reason for the operator (a missing, unparsable, oversized or unreadable file, a folder it could not read) and records a read failure as a diagnostic; the Swift path keeps its raw codes.
...
This path reads the Memory store only as the Swift list does, read-only, for recorded sessions whose files have gone; it schedules no analysis, cancels none, and leaves the due-work poll alone.
...
View tabs come back the first time their Workspace is in front in a process, each through the ordinary worker open; on this machine that waits until hided has opened the checkout's root, and on a device until its helper is ready and its catalog has moved its checkouts into their Projects (`ingest_host_established` and `refresh_device_catalog` ask again), because a read before the helper is refused and would mark every file unavailable, and a tab restored before the grouping names a Workspace the device no longer shows.
A Workspace does not record its tabs until it has restored them, nor while a restored file is still being read, so a Workspace whose files cannot be reached, or a quit before the reads land, keeps what it saved.
A restore opens into the Workspace in front, so it runs only while that is still the Workspace the last sync recorded; a front that moved since waits for the sync rather than filling one Workspace with another's tabs.
The reads land in any order, and once the last one has, the Workspace's View tabs are put back in the order they were saved.
A file that cannot be read comes back as a View tab with `unavailable_reason` and only a Close, and opening that file again once it is back reads it into the same tab; nothing is started in Herdr and no split or zoom is replayed.
