...
Which screen is showing is the shell's own location (`ui.ts`), not core state: the page starts on the Workspace the core has in front only when the core marks it `resumed`, the Workspace the views file last saw in front before this process started (D-11); a first run, a last Workspace that is gone, or any other front starts on Main once the device in front has settled (`startupScreen` in `navigation.ts`), and moving between screens creates or ends nothing.
