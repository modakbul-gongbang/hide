...
Each is one operator action and one frame, and it applies to the front Workspace only.
...
In web mode `file_open` places a document this way: one already displayed in the front Workspace focuses its most recently focused display, and pins it when the open is not a preview; otherwise a preview open takes the active area's preview display, and an ordinary open appends a pinned display to the active area.
`file_open` with `beside: true` is Open to the side: a pinned display of the document in the area next to the active one (right, then left, then down, then up among the tree's neighbours), a new area to the right when there is only one area (within the caps), or a focus of the display already in that area; it rides on `file_open` so the request still crosses hided's path boundary like any open.
...
Opening, restoring, renaming, closing and removing a device create and remove editor tabs by paths the layout does not see, so after every event the core reconciles the two: a display whose document tab is gone is removed, and an editor tab of a restored Workspace that no display shows, such as a Reopen Closed tab or a file the Explorer created, gets a pinned display in the active area.
...
Each display then moves through its own `state`: `waiting` while its Workspace's root cannot be read yet, with a `reason` naming the device state (`Waiting for <device> to connect`), then `opening` while its read is in flight, then `open` once it is bound to a document or diff tab, or `unavailable` with the read failure as its `reason`.
...
The request and its answer changed shape, so the helper protocol version moved with them (`hide-host/src/protocol.rs`): the core refuses a device helper from an earlier build and installs the one it carries, as for any protocol change.
