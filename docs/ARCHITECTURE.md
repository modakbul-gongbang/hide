...
View tabs come back the first time their Workspace is in front in a process, each through the ordinary worker open; on this machine that waits until hided has opened the checkout's root, and on a device until its helper is ready (`ingest_host_established` asks again), because a read before either is refused and would mark every file unavailable.
