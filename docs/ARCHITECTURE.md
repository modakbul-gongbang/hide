...
The sidebar's project and checkout rows carry a `⋯` menu, also on right-click (`web/src/RowMenu.tsx`, rules in `web/src/workspaceManage.ts`): Pin/Unpin for a registered local project, New worktree for a Git project on any device, a purpose for any checkout, and deletion for a linked worktree whose gate allows it on any device; an action a row cannot use is drawn disabled with its reason.
...
The Explorer, file tabs and History run on the device's own helper; worktree creation and deletion run through the device's Herdr and helper (see Device catalogs); reorder, reopen closed, find in pane and pin stay this machine's and are refused or disabled with the reason; a remote checkout's purpose is written through the core when the host's Herdr is 0.9.1 or newer.
...
A rename or move carries the draft to the new identity; a draft that cannot be stored, including when every stored draft together would pass 512 MiB, leaves editing alone and marks its tab "kept in this tab only" (D-14, B44).
...
A save whose answer was lost to a timeout or a dropped connection is `unknown`: it is never resent, further saves wait, and when the device's helper is ready the file is read back: the draft's revision means it saved, the base's means it has not reached the file yet (`file.save_not_applied`, worded as not yet, because a helper whose connection ended may still finish it and the next save then shows the conflict), anything else is a conflict.
