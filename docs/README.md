...
| Local image drop/paste, automatic handoff, exact-removal limits and provider authority | [DESIGN.md: Local image attachment shelf](../DESIGN.md#local-image-attachment-shelf), [attachment cost](PERFORMANCE_TESTING.md#local-image-attachment-cost-and-regression-ownership) | `attachments.rs`, `runtime/attachment_intents.rs`, `ImageAttachmentIngress.swift`, `ImageAttachmentFiles.swift`, `ImageAttachmentShelf.swift`, `viewport.rs`, `TerminalViewportSignal.swift` and boundary tests |
