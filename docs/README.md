...
| Web Main, Project Overview and Workspace screens, the Workspace layout, tools and View areas the core keeps per Workspace (split tree, displays bound to one document buffer, per-area preview, Open to the side, restore), drag and narrow-window presentation, the `documents` delta, and the pane relationship controls | [ARCHITECTURE.md: Workspaces in the web shell](ARCHITECTURE.md#workspaces-in-the-web-shell), [DESIGN.md: Web Workspace](../DESIGN.md#web-workspace) | `herdr-core/src/{workspace_views,view_layout}.rs`, `herdr-core/src/runtime/{workspace_view,view_areas}.rs`, `runtime/tests/{workspace_view,view_areas}.rs`, `web/src/{navigation,workspace,lineage,viewFocus,viewLayout,viewDrag}.ts`, `web/src/editor/sync.ts`, `web/src/{MainScreen,WorkspaceScreen,ViewAreas,Tools,TabBar,PaneRelations,Menu}.tsx`, `web/e2e/{s6,s7}.spec.ts` |
