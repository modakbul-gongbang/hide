...
| The hided daemon, its WebSocket contract, the web shell's panes and attach window, the `$HOME` boundary, and the shortcut registry | [ARCHITECTURE.md: hided and the WebSocket boundary](ARCHITECTURE.md#hided-and-the-websocket-boundary), [web echo and frame measurement](PERFORMANCE_TESTING.md#web-shell-echo-and-frame-measurement) | `hided/src/{server,boundary}.rs`, `hided/tests/handshake.rs`, `contracts/hided-ws.schema.json`, `web/src/{shortcuts,keyboard,close,resize,share}.ts`, `web/e2e/`, `scripts/web-shell-measure/` |
