...
- SC1. 크롬 제거: 운영자가 창을 열면 탭 스트립이 창의 맨 위에 붙어 있다.
  Actors: 운영자.
  Primary path: 창 상단에 시스템 타이틀바도 워크스페이스 헤더도 없고, 신호등 버튼 오른쪽에 사이드바 브랜드 헤더가, 사이드바 옆에 탭 스트립이 바로 이어진다. herdr 버전과 연결 상태는 상태 표시줄에서 읽힌다.
...
- R1. 터미널 컬럼의 54pt 헤더를 제거하고 탭 스트립이 컬럼의 첫 줄이 된다.
  ...
  herdr 버전, "herdr unavailable" 경고, 원격 대상 라벨은 상태 표시줄로 옮겨 헤더 제거 후에도 같은 정보가 같은 색 의미로 읽힌다.
...
| --- | --- | --- | --- |
| AC1 | 터미널 컬럼에서 탭 스트립 위에 아무 행도 없고, 사이드바와 오른쪽 패널 복원 버튼은 각 패널이 숨겨졌을 때 스트립 양끝에 나타나며, herdr 버전과 연결 경고와 원격 라벨은 상태 표시줄에서 읽힌다 | judged | 실행 중인 앱 캡처: 두 패널이 모두 보일 때, 왼쪽만 접었을 때, 오른쪽만 접었을 때, herdr 서버를 멈춘 상태 각각의 상단과 상태 표시줄 |
...
- T5. 터미널 컬럼 헤더를 제거하고 복원 버튼을 스트립 양끝으로, 버전과 연결 경고와 원격 라벨을 상태 표시줄로 옮긴다. Covers R1, R7, AC1, AC10, SC1. Depends on: none.
...
- design/principles.md 규칙 4: 유도 상태를 보여준다. 연결 경고와 버전은 헤더가 사라진 뒤에도 상태 표시줄에서 읽힌다.
