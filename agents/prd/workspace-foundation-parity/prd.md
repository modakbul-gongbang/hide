...
| D-19 | PRD 준비 완료 후 Mac mini의 Opus 5.5 pane을 Herdr로 직접 열어 implement 작업을 위임하되 문서 승인은 pending으로 남긴다. | 사용자: “ㅇㅇ 정리 다 되 implement로 mac mini에 opus5.5 로 pane 띄워서 작업하게 해줘!!”, “이 범위로 허용하고 구현 진행”, “그냥 너가 herdr로 직접 띄워서 해당 prd 읽고 implement 하라고 하자”; 이번 실행만 원격 dispatch 미지원에 대한 직접 실행 예외이며 최초 요청의 push/PR/merge·앱 교체·운영 설정/서비스 변경 금지는 유지한다. |
...
| D-24 | 재사용·사용자 흐름 순서·큰 위험·실제 확인·미결정만 짧게 계획하고 Observer는 누락·안전 경계·위험한 순서를 비동기로 검토한다. | 사용자 제안: “짧은 계획 + 비동기 방향 검토 + 위험한 흐름부터 실제 확인”; 가역적 작업은 계속하고 중요한 경계는 작은 동작으로 먼저 확인하며 요구사항·권한·구조 변경은 해당 작업만 멈춘다; 수정 중 영향 범위 검증, 최종 후보의 전체 필수 검증과 요구사항 충족 검토를 적용하고 사전 리뷰는 병렬로 허용하되 완료 근거는 최종 코드에 일치시킨다. |
