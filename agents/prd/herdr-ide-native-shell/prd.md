...
**위임 실행(`/please 승인!`)에서 사람 결정을 대신한 가정 (gap-audit 사이클 3, 사용자 사후 거부 가능)**

- ASM6. 워크벤치 인라인 편집의 **저장 실패 상태**를 인터뷰가 정하지 않았다(에이전트 편집 충돌 D-27만 다뤘다). 기본값으로 정한다: 쓰기 실패(권한 없음, 디스크 가득, 읽기 전용 전환, 편집 중 외부 삭제)는 사유를 그대로 담은 실패 표시를 띄우고, **편집 내용은 에디터에 그대로 남긴다**. 저장 실패로 사용자가 친 글자를 잃지 않는다. 실패를 조용히 삼키지 않는다(engineering 4).
- ASM7. **원격 SSH 연결 끊김의 재시도 정책**을 인터뷰가 정하지 않았다. D-38(herdr 서버 부재 시 자동 기동 없음, 명시적 사용자 조작만)과 같은 원칙으로 맞춘다: 끊기면 자동 재시도를 1회만 하고, 실패하면 무한 재시도 대신 '연결 끊김' 상태와 '다시 연결' 조작을 노출한다. 사용자가 모르는 사이에 계속 붙으려 하지 않는다.
- ASM8. **grab 페이로드의 프롬프트 인젝션**을 인터뷰가 고려하지 않았다. v1에서 수용 위험으로 둔다: 사용자가 요소를 직접 골라야만 주입되는 것이 경계다. 다만 주입 텍스트에 그 HTML·CSS가 **외부 웹 페이지에서 가져온 신뢰할 수 없는 내용**임을 명시하는 표식을 붙여, 에이전트가 그 안의 문장을 지시로 읽지 않게 한다. 별도 새니타이즈는 v1 비목표다.
- ASM9. **chromux 버전 호환성 정책**을 인터뷰가 정하지 않았다(herdr는 D-46의 `protocol` 게이팅이 있다). chromux는 별도 버전 체크를 두지 않는다: `--cdp-url` attach 실패를 포함한 모든 chromux 실패는 D-53대로 사유를 그대로 표시하며 조용히 넘어가지 않는다. herdr와 달리 chromux는 앱의 자료 구조를 해석하지 않아 버전 불일치가 부분 동작이 아니라 attach 실패로만 나타나기 때문이다.
