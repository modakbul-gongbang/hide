...
| D-11 | `Forget`은 즉시 future injection에서 제외하는 tombstone revision을 만들고 같은 app session에서 Undo할 수 있다. `Delete Memory data`는 Project의 derived session source와 locator, cursor, provenance, Memory item과 revision, search index, injection receipt를 모두 영구 삭제하므로 이 전체 범위를 설명하는 confirmation을 거친다. 두 동작 모두 raw provider session을 건드리지 않는다. | 인터뷰 D-07, D-26, Q20; design 6 |
...
| D-13 | Memory On 전에 현재 선택된 Background AI provider로 session 내용이 전송된다는 점, provider의 자체 retention과 deletion terms가 적용되며 전송 뒤 Hide가 provider 보관본의 삭제를 보장할 수 없다는 점, 파생 Memory가 local disk에 유지된다는 점, credential은 Memory에 저장하지 않는다는 점을 한 번 설명한다. 분석은 기존 `hide-ai`와 사용자가 로그인한 Claude Code 또는 Codex CLI만 사용한다. | 인터뷰 D-11, D-27, D-28; docs/AI_PROVIDERS.md |
...
| B7 | Memory가 Off인 Project에서 Memory tab을 처음 열면 기능 설명, 선택된 provider로의 session 전송, provider retention과 deletion terms 적용, 전송 뒤 Hide가 provider 보관본 삭제를 보장할 수 없다는 점, local persistence와 secret exclusion을 보여 주고 primary action 하나 `Turn on Memory`를 제공한다. 내부 engine이나 index 용어는 보이지 않는다. | D-01, D-09, D-13 |
...
| B30 | session analysis 전에 known secret pattern이 local redaction되고 secret 또는 credential candidate는 active Memory에 나타나지 않는다. Diagnosis와 AI logs 어디에도 prompt, transcript 또는 Memory body가 없다. | D-07, D-27 |
