...
/// Generated by `build.rs` from `macos/Sources/HerdrMacOS/Resources/herdr-bundle.json`, the
/// single source of the pin. There is no second literal to keep in step.
pub const BUNDLED_HERDR_VERSION: &str = crate::herdr_contract::BUNDLED_HERDR_VERSION;
...
mod tests {
    ...
    fn compatible_installed_runtime_wins_when_socket_is_absent() {
        assert_eq!(
            select_source(
                false,
                Some(installation(BUNDLED_HERDR_VERSION)),
                Some("/bundle/herdr".to_owned())
            )
            .expect("source"),
            HerdrSource::Installed(installation(BUNDLED_HERDR_VERSION))
        );
    }
