...
open class Highlightr
{
    ...
    public init?(highlightPath: String? = nil)
    {
        ...
        let containers = [Bundle.main, Bundle(for: Highlightr.self)]
            + Bundle.allBundles
            + Bundle.allFrameworks
        let bundle = containers.lazy.compactMap { container -> Bundle? in
            var roots = [
                container.bundleURL,
            ...
            return roots.lazy.compactMap { root in
                Bundle(url: root.appendingPathComponent(bundleName, isDirectory: true))
            }.first
        }.first
