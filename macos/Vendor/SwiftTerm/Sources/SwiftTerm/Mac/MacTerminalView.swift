...
///
/// Fills its background only under the glyphs used by each laid-out line rather
/// than across the line fragment or full frame. Line 1 already has an exclusion
/// path covering the portion occupied by pre-existing terminal text (e.g. the
/// prompt), so this drawing strategy leaves that area untouched while still
/// covering wrapped lines below.
