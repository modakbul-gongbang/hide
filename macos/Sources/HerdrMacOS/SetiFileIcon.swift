...
enum SetiFileIconCatalog {
    ...
    static func icon(fileName: String) -> SetiFileIcon {
        ...
        if [".gitignore", ".gitattributes", ".gitmodules", ".gitconfig"].contains(lowercased) {
            return icon("\u{E034}", "#6D8086", "point.3.connected.trianglepath.dotted")
        }
        if lowercased == "dockerfile" || lowercased == ".dockerignore" {
            return icon("\u{E025}", "#519ABA", "shippingbox")
        }
        if lowercased == "makefile" || lowercased == "cmakelists.txt" {
            return icon("\u{E05F}", "#E37933", "hammer")
        }
        if lowercased.hasSuffix(".lock") || ["package-lock.json", "yarn.lock", "pnpm-lock.yaml"].contains(lowercased) {
        ...
        case "rs": icon("\u{E082}", "#6D8086", "gearshape.2")
        ...
        case "txt", "log": icon("\u{E023}", "#D4D7D6", "doc.text")
        default: fallback
        }
    }
