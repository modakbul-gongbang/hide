...
struct OverviewGraph {
    ...
    struct Edge { let from: CGPoint; let to: CGPoint; let continuation: Bool }
    var rows: [Row] = []
    var edges: [Edge] = []
    ...
    init(history: CoreGitHistory?, checkouts: [CoreCheckoutSnapshot], expanded: Set<String>) {
        ...
        for row in rows {
            ...
            for parent in row.parents {
                ...
                edges.append(Edge(from: from, to: to, continuation: target == nil))
            }
        }
    ...
    var body: some View {
        ScrollView([.vertical, .horizontal]) {
            if let reason = history?.unavailableReason {
                    ...
                    ZStack(alignment: .topLeading) {
                        Canvas { context, _ in
                            for edge in graph.edges {
                                ...
                                context.stroke(path, with: .color(HideTheme.secondary),
                                               style: StrokeStyle(lineWidth: HideTheme.Layout.hairlineWidth,
                ...
                }.hideFont(size: HideTheme.Typography.caption).padding(.horizontal, HideTheme.spacingSM)
            }
