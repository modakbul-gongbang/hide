...
struct CoreProjectWorktrees: Decodable {
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case sharedGitDisk = "shared_git_disk", diskTotalBytes = "disk_total_bytes"
...
struct CoreGitWorktree: Decodable, Identifiable {
    ...
    /// fetch. Absent when there is no upstream, it is gone, or the count
    /// could not be read; `upstreamState` says which.
    let behindUpstream: Int?
    /// When a linked worktree was added; the main worktree has none.
    let createdAtUnixMS: Double?
    let unavailableReason: String?
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case behindUpstream = "behind_upstream", createdAtUnixMS = "created_at_unix_ms"
