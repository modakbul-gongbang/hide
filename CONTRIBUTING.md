...
The shared entrypoint runs the token, component ownership and counted control-policy checks, and the two design canvas checks, one that regenerates `design/hide.pen`'s tokens from `HideTheme.swift` and one that places every board by its name's band prefix; it performs static checks, not desktop interaction.
