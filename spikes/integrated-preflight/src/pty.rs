...
impl PtySession {
    ...
    pub fn write_bytes(&self, bytes: &[u8]) -> Result<()> {
        if bytes.is_empty() {
            return Err(anyhow!("stage=pty.write target=master cause=empty-payload"));
        }
        let mut writer = self
            .writer
            .lock()
    ...
    pub fn shutdown(&self) {
        let mut child_slot = match self.child.lock() {
            ...
            Err(_) => {
                eprintln!(
                    "event=pty.cleanup.failed stage=child.lock cause=poisoned-lock retryable=false"
                );
                return;
            }
        };
        ...
        match child.wait() {
            Ok(status) => eprintln!(
                "event=pty.cleanup.complete pid={process_id:?} exit_code={} signal={:?}",
                status.exit_code(),
                status.signal()
            ),
            Err(error) => eprintln!(
                "event=pty.cleanup.failed stage=child.wait pid={process_id:?} retryable=false error={error:?}"
            ),
        }
    }
}
